<main class="profilPatient d-flex flex-column justify-content-center align-items-center py-5">
        <form method="post" class="form p-4">
            <div class="lastname p-3">
                ...
                <input type="text" name="lastname" id="lastname" value="<?= $lastname ?? '' ?>" pattern="<?= REGEX_NAME ?>" placeholder="<?= $lastname ?>" readonly>
                <p class="error"><?= $alert['lastname'] ?? '' ?></p>
            </div>
            <div class="firstname p-3">
                <label for="firstname">Prénom : </label>
                <input type="text" name="firstname" id="firstname" value="<?= $firstname ?? '' ?>" pattern="<?= REGEX_NAME ?>" placeholder="<?= $firstname ?>" readonly>
                <p class="error"><?= $alert['firstname'] ?? '' ?></p>
            </div>
            <div class="birthdate p-3">
                <label for="birthdate">Date de Naissance : </label>
                <input type="date" name="birthdate" id="birthdate" value="<?= $birthdate ?? '' ?>" min="" max="" placeholder="<?= $birthdate?>" readonly>
                <p class="error"><?= $alert['birthdate'] ?? '' ?></p>
            </div>
            <div class="phone p-3">
                <label for="phone">Numéro de téléphone : </label>
                <input type="text" name="phone" id="phone" value="<?= $phone ?? '' ?>" pattern="<?= REGEX_PHONENUMBER ?>" placeholder="<?= $phone ?>" readonly>
                <p class="error"><?= $alert['phone'] ?? '' ?></p>
            </div>
            <div class="mail p-3">
                <label for="mail">Mail : </label>
                <input type="mail" name="mail" id="mail" value="<?= $mail ?? '' ?>" pattern="<?= REGEX_EMAIL ?>" placeholder="<?= $mail ?>" readonly>
