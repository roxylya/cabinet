<main class="profilPatient d-flex flex-column justify-content-center align-items-center py-5">
    <form method="post" class="form p-4">
        ...
        <div class="lastname p-3">
            ...
            <input type="text" name="lastname" id="lastname" value="<?= $profil->lastname ?? '' ?>" pattern="<?= REGEX_NAME ?>" readonly>
            <p class="error"><?= $alert['lastname'] ?? '' ?></p>
        </div>
        <div class="firstname p-3">
            <label for="firstname">Prénom : </label>
            <input type="text" name="firstname" id="firstname" value="<?= $profil->firstname ?? '' ?>" pattern="<?= REGEX_NAME ?>" readonly>
            <p class="error"><?= $alert['firstname'] ?? '' ?></p>
        </div>
        <div class="birthdate p-3">
            <label for="birthdate">Date de Naissance : </label>
            <input type="date" name="birthdate" id="birthdate" value="<?= $profil->birthdate ?? '' ?>" readonly>
            <p class="error"><?= $alert['birthdate'] ?? '' ?></p>
        </div>
        <div class="phone p-3">
            <label for="phone">Numéro de téléphone : </label>
            <input type="text" name="phone" id="phone" value="<?= $profil->phone ?? '' ?>" pattern="<?= REGEX_PHONENUMBER ?>" readonly>
            <p class="error"><?= $alert['phone'] ?? '' ?></p>
        </div>
        <div class="mail p-3">
            <label for="mail">Mail : </label>
            <input type="mail" name="mail" id="mail" value="<?= $profil->mail ?? '' ?>" pattern="<?= REGEX_EMAIL ?>" readonly>
